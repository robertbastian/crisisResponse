...
object TweetDao {
  ...
  private class TweetTable(tag: Tag) extends Table[Tweet](tag, "Tweet") {
    ...
    def toTweet(id: Option[Long], tx: String, at: String, ti: Timestamp, la: Double, lo: Double, te: Option[Double], re: Option[Double], co: Option[Double], pr: Option[Double], cl: Long): Tweet =
      Tweet(id, tx, at, ti,cl, if (la == 0 && lo == 0) None else Some((la, lo)), te, re, co, pr)
    def toTuple(t: Tweet) = Some((t.id,t.text,t.author,t.time,t.location.map(_._1).getOrElse(0.0),t.location.map(_._2).getOrElse(0.0),t.tension,t.recency,t.corroboration,t.proximity,t.collection))

    def * = (id.?, text, author, time, latitude, longitude, tension, recency, corroboration, proximity,collection).shaped <> ((toTweet _).tupled, (toTuple _))
  }
